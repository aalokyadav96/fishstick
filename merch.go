...
// 	// Respond with the list of merchs
// 	w.Header().Set("Content-Type", "application/json")
// 	json.NewEncoder(w).Encode(merchList)
// }
...
// 	RdxDel(fmt.Sprintf("merch:%s:%s", eventID, merchID)) // Invalidate the cache for the deleted merch
// 	RdxDel(fmt.Sprintf("merchlist:%s", eventID))         // Optionally invalidate the list cache
